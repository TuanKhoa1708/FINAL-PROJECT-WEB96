...
import { Upload, Button, Table, Progress, Typography, Tag, Space, Result } from "antd";
...
function UploadTeachers() {
  ...
  const handleFile = (file) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (data.length === 0) { showToast("No data found in the file.", "warning"); return; }
  ...
  const handleImport = async () => {
    ...
    for (let i = 0; i < validRows.length; i++) {
      try {
        await fetch("https://mindx-mockup-server.vercel.app/api/resources/teachers?apiKey=69ca789b3bb225ca08190764", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ name: validRows[i].name, subject: validRows[i].subject, email: validRows[i].email }),
        });
        success++;
      } catch {
        failed++;
  ...
  const reset = () => { setStage("upload"); setRows([]); setErrors([]); setProgress(0); setResults({ success: 0, failed: 0 }); };
  ...
  const previewColumns = [
    ...
    { title: "Subject", dataIndex: "subject", render: t => t ? <Tag color="blue">{t}</Tag> : <span style={{ color: "#94A3B8" }}>Missing</span> },
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <Title level={2} className="page-title">Upload Teachers via Excel</Title>
          <Text style={{ color: "var(--color-text-secondary)", fontSize: 14 }}>
            Bulk import teachers from an Excel spreadsheet
          </Text>
        </div>
        {stage !== "upload" && (
          <Button onClick={reset} icon={<DeleteOutlined />} style={{ borderRadius: 10 }}>Start Over</Button>
        )}
      </div>
      ...
      <div className="step-indicator" style={{ marginBottom: 28 }}>
        {[
          ...
          const status = stageIdx > s.step ? "done" : stageIdx === s.step ? "active" : "pending";
          return (
            <div key={s.step} className="step-item">
              <div className={`step-circle ${status}`}>{status === "done" ? "✓" : s.step}</div>
              <div className={`step-label ${status}`}>{s.label}</div>
              {idx < 3 && <div className={`step-line ${stageIdx > s.step ? "done" : ""}`} />}
            </div>
          );
        })}
      </div>
          ...
          <div style={{ maxWidth: 600, margin: "0 auto" }}>
            <div className="upload-dragger-custom">
              <Dragger beforeUpload={handleFile} accept=".xlsx,.xls" showUploadList={false} style={{ padding: "20px 0" }}>
                ...
                <p style={{ fontSize: 16, fontWeight: 600, color: "var(--color-text-primary)", marginBottom: 8 }}>
            ...
            <div style={{ marginTop: 24, background: "#F8FAFC", borderRadius: 10, padding: 16 }}>
              <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 8, color: "var(--color-text-primary)" }}>📋 Required columns in your Excel file:</div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {[{ name: "name", required: true }, { name: "subject", required: true }, { name: "email", required: false }].map(col => (
                  <Tag key={col.name} color={col.required ? "blue" : "default"} style={{ borderRadius: 6 }}>
      ...
      {stage === "preview" && (
        <div>
          <div style={{ display: "flex", gap: 16, marginBottom: 20 }}>
            <div className="premium-card" style={{ flex: 1, padding: 16, display: "flex", gap: 12, alignItems: "center" }}>
              <div style={{ fontSize: 24 }}>📄</div>
              <div>
                <div style={{ fontSize: 11, color: "var(--color-text-muted)", fontWeight: 600, textTransform: "uppercase" }}>Total Rows</div>
                <div style={{ fontSize: 24, fontWeight: 800 }}>{rows.length}</div>
              </div>
            </div>
            <div className="premium-card" style={{ flex: 1, padding: 16, display: "flex", gap: 12, alignItems: "center" }}>
              <div style={{ fontSize: 24 }}>✅</div>
              <div>
                <div style={{ fontSize: 11, color: "var(--color-text-muted)", fontWeight: 600, textTransform: "uppercase" }}>Valid</div>
                <div style={{ fontSize: 24, fontWeight: 800, color: "#22C55E" }}>{rows.filter(r => r._valid).length}</div>
              </div>
            </div>
            <div className="premium-card" style={{ flex: 1, padding: 16, display: "flex", gap: 12, alignItems: "center" }}>
              <div style={{ fontSize: 24 }}>❌</div>
              <div>
                <div style={{ fontSize: 11, color: "var(--color-text-muted)", fontWeight: 600, textTransform: "uppercase" }}>Errors</div>
                <div style={{ fontSize: 24, fontWeight: 800, color: "#EF4444" }}>{errors.length}</div>
              </div>
            </div>
          </div>

          <div className="premium-card">
            <Table dataSource={rows} columns={previewColumns} rowKey="key"
      ...
      {stage === "importing" && (
        <div className="premium-card" style={{ padding: 60, textAlign: "center" }}>
          ...
          <div style={{ fontSize: 20, fontWeight: 700, marginBottom: 8 }}>Importing Teachers...</div>
          <div style={{ color: "var(--color-text-secondary)", marginBottom: 28 }}>Please wait while we upload the data.</div>
          <Progress percent={progress} status="active" strokeColor={{ from: "#1E40AF", to: "#6366F1" }} style={{ maxWidth: 400, margin: "0 auto" }} />
          <div style={{ marginTop: 12, color: "var(--color-text-muted)", fontSize: 14 }}>{progress}% complete</div>
        </div>
      )}
      ...
      {stage === "done" && (
        <div className="premium-card" style={{ padding: 40 }}>
          <Result
            ...
            extra={[
              <Button type="primary" key="restart" onClick={reset} style={{ borderRadius: 10 }}>Upload Another File</Button>
            ]}
          />
        </div>
      )}
