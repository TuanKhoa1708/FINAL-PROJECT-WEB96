...
function ManageSchedule() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const schedules = sd.data || [];
      const teacherList = td.data || [];
  ...
  const getTeacherName = (id) =>
    teachers.find((t) => String(t.id) === String(id))?.name || `Teacher #${id}`;
  ...
  const handleSave = async () => {
    ...
    if (checkOverlap(editingItem)) {
      Modal.confirm({
        ...
        content:
          "This teacher or class already has a slot at the same time. Do you want to continue anyway?",
        okText: "Save Anyway",
        cancelText: "Go Back",
        centered: true,
        onOk: () => doSave(),
  ...
  const doSave = async () => {
    ...
    try {
      if (editingItem._id) {
        await apiFetch(`/admin/schedules/${editingItem._id}`, {
          method: "PUT",
  ...
  const handleDelete = (_id) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await apiFetch(`/admin/schedules/${_id}`, { method: "DELETE" });
  ...
  schedule.forEach((item) => {
    const period = PERIODS.find(
      (p) =>
        p.time === item.time ||
        p.label === item.time ||
        item.time?.includes(p.id.toString()),
    );
  ...
  return (
    <div>
                ...
                <tr>
                  ...
                  {DAYS.map((d, i) => (
                    <th key={d}>
                      {d}
                      <div
                        style={{ fontSize: 10, opacity: 0.7, fontWeight: 400 }}
                      >
                        {DAY_SHORT[i]}
                      </div>
                    </th>
                      ...
                      return (
                        <td key={day} style={{ padding: 6 }}>
                          {items.length === 0 ? (
                            ...
                            >
                              {items.map((item) => {
                                const c = SUBJECT_COLORS[item.subject] || {
                                  bg: "#F1F5F9",
                                  text: "#475569",
                                  border: "#CBD5E1",
                                };
          ...
          <Table
            ...
            columns={[
              ...
              { title: "Time", dataIndex: "time" },
              {
                ...
                dataIndex: "room",
                render: (t) =>
                  t || <span style={{ color: "#94A3B8" }}>TBD</span>,
              },
              {
                title: "Actions",
                align: "right",
      ...
      <Modal
        ...
        title={editingItem?._id ? "Edit Schedule Entry" : "Add New Class"}
        okText="Save"
        centered
        width={480}
      >
          ...
          >
            ...
            <div>
              ...
              <Select
                size="large"
                style={{ width: "100%" }}
                value={editingItem.subject || undefined}
                placeholder="Select subject"
                onChange={(v) => setEditingItem({ ...editingItem, subject: v })}
              >
                {SUBJECTS.map((s) => (
            ...
            <div>
              ...
              <Select
                size="large"
                style={{ width: "100%" }}
                value={editingItem.teacherId || undefined}
                placeholder="Select teacher"
                onChange={(v) =>
                  setEditingItem({ ...editingItem, teacherId: v })
                }
              >
                {teachers.map((t) => (
                  <Option key={t._id} value={String(t.id)}>
                    {t.name} — {t.subject}
                  </Option>
                ))}
              </Select>
            ...
            <div>
              ...
              <Select
                size="large"
                style={{ width: "100%" }}
                value={editingItem.day || undefined}
                placeholder="Select day"
                onChange={(v) => setEditingItem({ ...editingItem, day: v })}
              >
                {DAYS.map((d) => (
            ...
            <div>
              ...
              <Select
                size="large"
                style={{ width: "100%" }}
                value={editingItem.time || undefined}
                placeholder="Select period"
                onChange={(v) => setEditingItem({ ...editingItem, time: v })}
              >
                {PERIODS.map((p) => (
                  <Option key={p.id} value={`${p.label} (${p.time})`}>
                    {p.label} — {p.time}
                  </Option>
                ))}
              </Select>
