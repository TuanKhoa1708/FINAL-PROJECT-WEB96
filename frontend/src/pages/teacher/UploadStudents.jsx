...
function UploadStudents() {
  ...
  const handleFile = (file) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        setStage("preview");
        showToast(
          `${processed.length} rows loaded, ${processed.filter((r) => r._valid).length} valid.`,
          "info",
        );
      } catch {
  ...
  const handleImport = async () => {
      ...
      try {
        await apiFetch("/admin/students", {
          ...
          body: JSON.stringify({
            ...
            class: validRows[i].class,
            email: validRows[i].email,
            homeroomTeacherId: validRows[i].homeroomTeacherId,
          }),
        });
        success++;
      } catch {
    ...
    setStage("done");
    showToast(
      `Import done: ${success} students added.`,
  ...
  return (
    <div>
      ...
      <div className="step-indicator" style={{ marginBottom: 28 }}>
        {["Upload File", "Preview & Validate", "Importing", "Done"].map(
          (label, idx) => {
            ...
            return (
              <div key={label} className="step-item">
                ...
                {idx < 3 && (
                  <div
                    className={`step-line ${stageIdx > step ? "done" : ""}`}
                  />
                )}
              </div>
            );
          ...
          <div style={{ maxWidth: 600, margin: "0 auto" }}>
            <div className="upload-dragger-custom">
              ...
              >
                ...
                >
                  or click to browse — <Tag>.xlsx</Tag> <Tag>.xls</Tag>
                </p>
              </Dragger>
            </div>
            <div
            ...
            >
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {[
                  ...
                  { n: "class", r: true },
                  { n: "email", r: false },
                  { n: "homeroomTeacherId", r: false },
                ].map((c) => (
                  <Tag key={c.n} color={c.r ? "blue" : "default"}>
                    {c.n} {c.r ? "*" : "(optional)"}
                  </Tag>
                ))}
              </div>
        ...
        <div>
          <div style={{ display: "flex", gap: 16, marginBottom: 20 }}>
            {[
              ...
              {
                label: "Valid",
                value: rows.filter((r) => r._valid).length,
                icon: "✅",
                color: "#22C55E",
              },
              {
          ...
          >
            ...
            <Button
              ...
              disabled={!rows.filter((r) => r._valid).length}
