...
function TeacherSchedule() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await apiFetch("/teacher/schedule");
        const data = await res.json();
        const all = data.data || [];
        // Filter for this teacher's classes
        const mine = all.filter(
          (s) => String(s.teacherId) === String(user?.id),
        );
        setSchedule(mine);
      } catch (err) {
        console.error(err);
  ...
  const getCellItems = (day, periodId) =>
    schedule.filter(
      (s) =>
        s.day === day &&
        (s.time?.includes(periodId.toString()) ||
          s.time?.includes(PERIODS.find((p) => p.id === periodId)?.label)),
    );
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 16, marginBottom: 24 }}>
        {[
          ...
          {
            label: "Days Active",
            value: [...new Set(schedule.map((s) => s.day))].length,
            icon: "📅",
          },
          {
            label: "Unique Classes",
            value: [...new Set(schedule.map((s) => s.class))].length,
            icon: "🏫",
          },
        ].map((card) => (
          <div
          ...
          {schedule.length === 0 ? (
            <div className="empty-state">
              ...
              <div className="empty-sub">
                Ask your administrator to assign you to classes.
              </div>
            </div>
          ) : (
            <div
          ...
          <Table
            ...
            columns={[
              ...
              },
              {
                title: "Day",
                dataIndex: "day",
                sorter: (a, b) => DAYS.indexOf(a.day) - DAYS.indexOf(b.day),
              },
