            ...
            <Dropdown menu={userMenu} trigger={['click']} placement="bottomRight">
              <div style={{ display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }}>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: "var(--color-text-primary)" }}>{user?.username || "Teacher User"}</div>
                  <div style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>Instructor</div>
                </div>
                <Avatar icon={<UserOutlined />} style={{ backgroundColor: "var(--color-secondary)" }} />
              </div>
            </Dropdown>
          </div>
        </header>
